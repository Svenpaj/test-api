...
import (
	...
	"github.com/Svenpaj/test-api/model"
...
func (r *RedisRepo) DeleteByID(ctx context.Context, id uint64) error {
	...
	if err := txn.SRem(ctx, "orders", key).Err(); err != nil {
		txn.Discard()
		return fmt.Errorf("failed to revmove from orders set: %w", err)
	}
...
func (r *RedisRepo) FindAll(ctx context.Context, page FindAllPage) (FindResult, error) {
	...
	for i, x := range xs {
		...
		if err != nil {
			return FindResult{}, fmt.Errorf("failed to devode order json: %w", err)
		}
